...
class FxStore:
    ...
    def upsert_candles(self, candles: list[Candle]) -> int:
        ...
        assert candles is not None, "candles must be a list, not None (pass [] to no-op)"
        if not candles:
            return 0
        assert all(c.instrument for c in candles), "every candle needs a non-empty instrument"
