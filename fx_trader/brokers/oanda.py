...
CONFIRMED BUGS, FOUND AND FIXED (first real run against a practice account):
1. fetch_candles built request timestamps with Python's .isoformat(), which
   renders UTC as "...+00:00". OANDA's v20 API expects a literal "Z" suffix.
   Fixed via _format_oanda_time().
2. fetch_candles sent from + to + count together - OANDA's spec says count
   must not be specified when both start and end are given. Fixed by never
   sending `to`; paginate via `from`+`count` only, filter against `end`
   client-side.

get_quote and place_market_order have never been exercised against a real
account - treat with elevated suspicion given the above history.
"""
...
class OandaBroker(BrokerAdapter):
    ...
    def fetch_candles(
        ...
        """
        Paginates with `from`+`count` only (never `to`) per OANDA's OpenAPI
        spec - see module docstring for why. Filters each response against
        `end` client-side instead. Pagination loop has an explicit ceiling
        (MAX_PAGES) so a misbehaving cursor can't loop forever.
        """
        assert start < end, "fetch_candles requires start < end"
        all_candles: list[Candle] = []
        cursor = start
        pages_fetched = 0

        while cursor < end and pages_fetched < self.MAX_PAGES:
            pages_fetched += 1
            raw_candles = self._fetch_one_page(instrument, granularity, cursor)
            if not raw_candles:
                break

            all_candles.extend(self._parse_candle_page(raw_candles, instrument, granularity, end))
    ...
    MAX_PAGES = 500  # explicit ceiling: 500 * 5000 candles is far beyond any realistic single fetch

    def _fetch_one_page(self, instrument: str, granularity: str, cursor: datetime) -> list[dict]:
        params = {
            "price": "BA",
    ...
    def _parse_candle_page(
        self, raw_candles: list[dict], instrument: str, granularity: str, end: datetime
    ) -> list[Candle]:
        """One page's raw JSON candles -> Candle objects, dropping incomplete
        candles and anything past `end`."""
        assert raw_candles, "_parse_candle_page requires a non-empty page"
        batch = []
        for rc in raw_candles:
            rc_time = datetime.fromisoformat(rc["time"].replace("Z", "+00:00"))
            if rc_time > end or not rc.get("complete", True):
                continue
