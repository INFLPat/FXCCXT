...
class CcxtBroker(BrokerAdapter):
    ...
    MAX_PAGES = 500  # explicit ceiling: far beyond any realistic single fetch
    ...
    def fetch_candles(
        ...
        while since_ms < end_ms and pages_fetched < self.MAX_PAGES:
            pages_fetched += 1
            ...
            all_candles.extend(self._map_ohlcv_batch(batch, instrument, granularity, end_ms))
    ...
    def _map_ohlcv_batch(
        self, batch: list, instrument: str, granularity: str, end_ms: int
    ) -> list[Candle]:
        """One ccxt OHLCV page -> Candle objects, stopping at end_ms. No
        separate historical bid/ask available - see module docstring."""
        assert batch, "_map_ohlcv_batch requires a non-empty batch"
        candles = []
