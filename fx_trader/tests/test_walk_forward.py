...
class AlwaysBuyStrategy(Strategy):
    """Buys exactly once across its lifetime (state survives warm_start) and
    holds forever. If should_trade=False, never trades."""
...
def test_selection_and_position_continuity_across_boundary():
    """One window, in_sample_size=3, out_of_sample_size=2. Prices rise
    during in-sample (100->101->103), so buying should win selection. The
    winning position - bought at 100, still open when in-sample ends - must
    carry into out-of-sample (104, 102) rather than vanishing."""
    ...
    assert len(oos_trades) == 1, f"expected exactly 1 trade carried into out-of-sample, got {len(oos_trades)}"
    ...
    expected_oos_pnl = (102.0 - 100.0) * 1.0
...
def test_no_lookahead_between_in_sample_and_out_of_sample():
    """Every window's out-of-sample slice starts strictly after its
    in-sample slice ends - proves windows don't overlap into the future."""
