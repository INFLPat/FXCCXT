...
Tests CcxtBroker's own logic (OHLCV-to-Candle mapping, pagination, quote/
order/balance handling) against a fake exchange object. Does NOT verify a
real ccxt exchange actually behaves this way - see brokers/ccxt_broker.py.

Run: python -m tests.test_ccxt_broker
"""
...
def test_fetch_candles_pagination_and_mapping():
    page1 = [
        [1704067200000, 42000.0, 42100.0, 41900.0, 42050.0, 12.5],
        [1704070800000, 42050.0, 42200.0, 42000.0, 42150.0, 9.8],
    ]
    page2 = [
        [1704074400000, 42150.0, 42300.0, 42100.0, 42250.0, 7.1],
    ]
    ...
    assert candles[0].bid_close == candles[0].ask_close == 42050.0
    ...
    assert fake.ohlcv_calls[1]["since"] == 1704070800000 + 3600 * 1000
...
def test_get_quote_falls_back_to_last_price():
    ...
    assert quote.bid == quote.ask == 42005.0
...
def test_get_account_balance_uses_quote_currency():
    ...
    assert broker_missing_currency.get_account_balance() == 0.0
