...
1. Hand-verified math against a directly-constructed 3x3 grid with known
   scores (bypassing the engine) - proves neighbor_gap, heatmap_grid,
   best/worst, marginal_effect.
2. Integration test with the real engine and a trivial strategy, proving
   run_sensitivity_analysis wires everything together, including gracefully
   handling a parameter combination the strategy rejects.

Run: python -m tests.test_sensitivity
"""
...
def _make_3x3_spike_result() -> SensitivityResult:
    """3x3 grid, param_a/param_b in [0,1,2]. Every point scores 1.0 EXCEPT
    the dead center (1,1), which scores 10.0 - a deliberate isolated spike."""
    grid_points = []
    for a in (0, 1, 2):
        for b in (0, 1, 2):
...
def test_best_worst_and_heatmap_grid():
    ...
    assert abs(result._score(worst[0]) - 1.0) < 1e-9
...
class ToggleStrategy(Strategy):
    ...
    should_trade is None - tests that run_sensitivity_analysis handles a
    rejected combination gracefully instead of crashing."""
