...
1. Hand-verified percentile/CI/probability math against a constructed
   BootstrapResult with known values.
2. Shuffle mode's core invariant: total return and win rate are EXACTLY
   order-invariant - proven, not assumed.
3. Ground-truth check: exhaustively enumerate every ordering of a small
   trade set with itertools.permutations and compare against the Monte
   Carlo estimate.
...
def _closed_trade(pnl: float, idx: int) -> Trade:
    ...
    1 unit - realized_pnl() = exit_price - entry_price."""
    return Trade(
